...
import { registerCashPayment, registerMayoristaPayment, registerDeudaAnterior, registerNotaDebito, saveReciboPdf, ensureReciboNumero, findReciboByNumero, deletePayment, voidPayment } from '@/services/payments-service'
import {
...
export const sellersApi = {
  ...
  async pagarComisionesPeriodo(sellerId: string, sellerName: string, desde: Date, hasta: Date, input?: PagoComisionInput): Promise<PagoComision> {
    return pagarComisionesPeriodo(sellerId, sellerName, desde, hasta, input)
  },
...
export const cobranzasApi = {
  ...
  async getAllClientsBySeller(sellerId: string) {
    return getDebtClients(sellerId, true, true)
  },
  ...
  async uploadComprobantePagoAdmin(data: {
    clientId: string
    amount: number
    notes?: string
    file: File
    ...
    return uploadComprobantePagoAdmin(data)
  },
